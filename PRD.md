...
**Deliberately not done.** No Content-Security-Policy — Next's inline hydration scripts and
GSAP's runtime style injection make a strict policy too likely to ship a blank page. No
honeypot or captcha on the contact form; rate limiting and length bounds cover the
realistic abuse.

**Known gap.** Tab can move focus into the embedded video iframe, after which Escape no
longer closes the project modal — the focus-trap selector doesn't match iframes. Minor,
long-standing, unfixed.
...
- Fixing the iframe focus-trap gap in §9
- A test suite
